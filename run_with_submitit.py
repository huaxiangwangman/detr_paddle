...
def parse_args():
    ...
    parser = argparse.ArgumentParser('Submitit for detection', parents=[
        detection_parser])
    parser.add_argument('--ngpus', default=8, type=int, help=\
        'Number of gpus to request on each node')
    parser.add_argument('--nodes', default=4, type=int, help=\
        'Number of nodes to request')
    parser.add_argument('--timeout', default=60, type=int, help=\
        'Duration of the job')
    parser.add_argument('--job_dir', default='', type=str, help=\
...
class Trainer(object):
    ...
    def _setup_gpu_args(self):
        ...
        self.args.output_dir = Path(str(self.args.output_dir).replace('%j',
            str(job_env.job_id)))
