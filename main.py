...
from x2paddle.torch2paddle import DistributedSampler
import datasets
...
def get_args_parser():
    ...
    parser.add_argument('--dilation', action='store_true', help=\
    ...
    parser.add_argument('--enc_layers', default=6, type=int, help=\
...
def main(args):
    ...
    param_dicts = [{'params': [p for n, p in model_without_ddp.
        named_parameters() if 'backbone' not in n and p.requires_grad]}, {
        'params': [p for n, p in model_without_ddp.named_parameters() if 
        'backbone' in n and p.requires_grad], 'lr': args.lr_backbone}]
    ...
    batch_sampler_train = paddle.io.BatchSampler(sampler_train, args.
        batch_size, drop_last=True, dataset=None)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        log_stats = {**{f'train_{k}': v for k, v in train_stats.items()},
            **{f'test_{k}': v for k, v in test_stats.items()}, 'epoch':
