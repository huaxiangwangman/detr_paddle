...
def _make_detr(backbone_name: str, dilation=False, num_classes=91, mask=False):
    ...
    detr = DETR(backbone_with_pos_enc, transformer, num_classes=num_classes,
        num_queries=100)
    if mask:
...
def detr_resnet50_dc5(pretrained=False, num_classes=91,
    ...
    if pretrained:
        checkpoint = paddle.load(
            'https://dl.fbaipublicfiles.com/detr/detr-r50-dc5-f0fb7ef5.pdiparams'
            )
        model.load_state_dict(checkpoint['model'])
    if return_postprocessor:
        return model, PostProcess()
...
def detr_resnet50_dc5_panoptic(pretrained=False, num_classes=250, threshold
    =0.85, return_postprocessor=False):
    """
    DETR-DC5 R50 with 6 encoder and 6 decoder layers.
...
def detr_resnet101_panoptic(pretrained=False, num_classes=250, threshold=\
    0.85, return_postprocessor=False):
    """
    DETR-DC5 R101 with 6 encoder and 6 decoder layers.
   ...
   threshold is the minimum confidence required for keeping segments in the prediction
    ...
    if pretrained:
        checkpoint = paddle.load(
            'https://dl.fbaipublicfiles.com/detr/detr-r101-panoptic-40021d53.pdiparams'
            )
        model.load_state_dict(checkpoint['model'])
    if return_postprocessor:
        return model, PostProcessPanoptic(is_thing_map, threshold=threshold)
