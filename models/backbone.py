...
class FrozenBatchNorm2d(paddle.nn.Layer):
    ...
    def __init__(self, n):
        ...
        self.register_buffer('weight', paddle.ones(n).requires_grad_(False))
        self.register_buffer('bias', paddle.zeros(n).requires_grad_(False))
        self.register_buffer('running_mean', paddle.zeros(n).requires_grad_
            (False))
        self.register_buffer('running_var', paddle.ones(n).requires_grad_(
            False))
...
class BackboneBase(nn.Layer):
    ...
    def __init__(self, backbone: nn.Layer, train_backbone: bool,
    ...
    def forward(self, tensor_list: NestedTensor):
        ...
        for name, x in xs.items():
            ...
            mask = F.interpolate(m[None].float(), size=x.shape[-2:]).to(torch
                .bool)[0]
