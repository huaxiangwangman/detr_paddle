...
from .segmentation import PostProcessSegm
...
class DETR(nn.Layer):
    ...
    def forward(self, samples: NestedTensor):
        ...
        """
        if isinstance(samples, (list, torch2paddle.create_tensor)):
            samples = nested_tensor_from_tensor_list(samples)
        features, pos = self.backbone(samples)
        src, mask = features[-1].decompose()
        assert mask is not None
        hs = self.transformer(self.input_proj(src), mask, self.query_embed.
            weight, pos[-1])[0]
    ...
    def _set_aux_loss(self, outputs_class, outputs_coord):
        return [{'pred_logits': a, 'pred_boxes': b} for a, b in zip(
...
class PostProcess(nn.Layer):
    ...
    def forward(self, outputs, target_sizes):
        ...
        scale_fct = paddle.stacks([img_w, img_h, img_w, img_h], axis=1)
